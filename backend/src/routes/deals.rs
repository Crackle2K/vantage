...
async fn create_deal(
    ...
    let mut deal_doc = doc! {
        ...
        "title": security::sanitize_text(&payload.title, 200),
        "is_active": true,
        "created_at": now.to_rfc3339(),
        "updated_at": now.to_rfc3339(),
    };
    if let Some(desc) = &payload.description {
        deal_doc.insert("description", security::sanitize_text(desc, 1000));
    }
    if let Some(pct) = payload.discount_percent {
        deal_doc.insert("discount_percent", pct);
    }
    if let Some(op) = payload.original_price {
        deal_doc.insert("original_price", op);
    }
    if let Some(dp) = payload.deal_price {
        deal_doc.insert("deal_price", dp);
    }
...
async fn update_deal(
    ...
    let businesses: mongodb::Collection<Document> = state.db.mongo.collection("businesses");
    let biz = businesses
        .find_one(doc! { "_id": existing.get_str("business_id").unwrap_or("") })
        .await?
        .unwrap_or_default();
    if biz.get_str("owner_id").unwrap_or("") != auth_user.id && auth_user.role != "admin" {
        return Err(AppError::Forbidden("Not the business owner".into()));
    ...
    if let Some(t) = &payload.title {
        update.insert("title", security::sanitize_text(t, 200));
    }
    if let Some(d) = &payload.description {
        update.insert("description", security::sanitize_text(d, 1000));
    }
    if let Some(p) = payload.discount_percent {
        update.insert("discount_percent", p);
    }
    if let Some(a) = payload.is_active {
        update.insert("is_active", a);
    }

    col.update_one(doc! { "_id": oid }, doc! { "$set": update })
        .await?;
